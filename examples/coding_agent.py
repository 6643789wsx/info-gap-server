...
# Run application
REQUEST = Request(
    this_paper_should_be="about software architecture recovery with large language models and multi - agent",
    accepted_reason_format="The exhaustive list of agents is `your_agents`. The agents communicate by `your_method`. The agents are designed for `your_purpose` in the context of software architecture recovery.",
    unaccepted_reason_format="The paper is not about software architecture recovery with large language models and multi - agent, because `your_reason`.",
    examples=[
        Example(
            ...
            accepted=False,
            reason="The paper is not about software architecture recovery with large language models and multi - agent, because `although it's about LLM, there's no mention of multi - agent or software architecture recovery`.",
        ),
        Example(
            ...
            accepted=False,
            reason="The paper is not about software architecture recovery with large language models and multi - agent, because `although it's about software development with LLM, there's no mention of multi - agent or software architecture recovery`.",
        ),
        Example(
            title="OmniAgent: A multi - agent system for software architecture recovery",
            summary="The paper presents OmniAgent, a multi - agent system for software architecture recovery. We propose a message - pool architecture for agents to communicate with each other. Agents include Architecture Analyzer, Component Identifier, Dependency Mapper, and Documentation Generator. The agents are designed for improving the accuracy of software architecture recovery.",
            accepted=True,
            reason="The exhaustive list of agents is `Architecture Analyzer, Component Identifier, Dependency Mapper, and Documentation Generator`. The agents communicate by `message - pool architecture`. The agents are designed for `improving the accuracy of software architecture recovery`.",
        ),
    ],
)

# # automatic software development with large language models and multi-agent
# REQUEST = Request(
